...
def test_view(client, article):
    response = client.get(reverse("feeds:article", kwargs={"code": article.code}), follow=False)
    assert redirects_to(response, article.url)
    assert article_was_clicked(article)


def test_click_view(client, article):
    response = client.post(reverse("feeds:article-click"), data={"code": article.code})
