...
@pytest.fixture
def url(source):
    return reverse("feeds:source", args=(source.slug,))
...
def test_errors(client):
    response = client.get(reverse("feeds:source", args=("no-such-source",)), follow=True)
    assert response.status_code == 200
    assert redirects_to(response, reverse("feeds:sources"))
