...
@pytest.fixture
def url(tag):
    return reverse("feeds:tag", args=(tag.slug,))
...
def test_errors(client):
    response = client.get(reverse("feeds:tag", args=("no-such-tag",)), follow=True)
    assert response.status_code == 200
    assert redirects_to(response, reverse("feeds:tags"))
    assert warning_message_shown(response)


def test_tag_for_author(client, tag, articles):
    author = Author.objects.first()
    url = reverse("feeds:tag-author", args=(tag.slug, author.slug))
    response = client.get(url)
...
def test_tag_for_source(client, tag, articles):
    ...
    url = reverse("feeds:tag-source", args=(tag.slug, source.slug))
    response = client.get(url)
